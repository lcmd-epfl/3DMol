...
for config_file in glob('configs/config-*-*-*-????????-????????.dat'):
    ...
    for split_type in ['random', 'HS_LS_same_fold', 'train_valid']:
        ...
        if split_type == 'random':
            splitter = f'test:data/OctaMK/splits/random_{size}/${{FOLD}}_test_indices.txt'
            array = '0-9'
            if short_dataset=='OctaFull':
            ...
            splitter = f'test:data/OctaMK/splits/train_valid_{size}/${{FOLD}}_test_indices.txt'
            array = '0'
            train_frac_bash = 'TRAIN_FRAC=0.599'
        elif split_type == 'HS_LS_same_fold':
            splitter = f'test:data/OctaMK/splits/HS_LS_same_fold_{size}/${{FOLD}}_test_indices.txt'
            array = '0-9'
...
--experiment_name 3DMol-OctaMK-cv \\
--CV 1 \\
--num_epochs 512 \\
--splitter {splitter} \\
